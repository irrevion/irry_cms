	...
	<div class="box">
		...
		<form action="" method="post" enctype="multipart/form-data" class="form-std" autocomplete="off" role="form" id="formUserInfo">
			...
			<div class="box-body">
				<div class="row">
					<div class="col-md-6">
						<div class="userpic-big pad">
							<?php if (empty($user['avatar'])) { ?>
							<?=view::gravatar($user['login'], 256, [
								'class' => 'profile-user-img img-responsive img-circle',
								'alt' => CMS::t('cms_user_avatar')
							]);?>
							<?php } else { ?>
							<img src="<?=utils::safeEcho($avatarDirUrl.$user['avatar'], 1);?>" alt="<?=CMS::t('cms_user_avatar');?>" class="profile-user-img img-responsive img-circle" />
							<?php } ?>
						</div>
						...
						<div class="form-group">
							...
							<select name="lang" class="form-control">
								...
								<?php
									$current_lng = (isset($_POST['lang'])? @(string)$_POST['lang']: $user['lang']);
									if (!empty($langs) && is_array($langs)) foreach ($langs as $lng) {
								?><option value="<?=$lng['language_dir'];?>"<?=(($lng['language_dir']==$current_lng)? ' selected="selected"': '');?>><?=CMS::t($lng['language_dir']);?> (<?=utils::safeEcho($lng['language_name'], 1);?>)</option><?php
									}
								?>
							</select>
						</div>
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<input type="text" name="name" value="<?=utils::safePostValue('name', $user['name'], 1);?>" class="form-control" autocomplete="off" />
						</div>
						...
						<div class="form-group">
							...
							<select name="role" class="form-control">
								...
								<?php
									$current_role = (isset($_POST['role'])? @(string)$_POST['role']: $user['role']);
									if (!empty(CMS::$roles) && is_array(CMS::$roles)) foreach (CMS::$roles as $role=>$allowed_pages) {
								?><option value="<?=$role;?>"<?=(($role==$current_role)? ' selected="selected"': '');?>><?=CMS::t('cms_users_role_'.$role);?></option><?php
									}
								?>
							</select>
						</div>
						...
						<div class="form-group">
							<label><?=CMS::t('cms_user_new_pwd');?></label>
